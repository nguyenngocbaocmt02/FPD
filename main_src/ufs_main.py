...
def ufs_solve_dataset(dp, cfg, queries_num, seed, save_path):
    np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        for seed in seeds:            
            ...
            save_path = os.path.join(os.path.join(os.path.join(result_folder_path, "ufs"), dataset), str(seed) + ".csv")
            if os.path.exists(save_path):
                continue
            dp, config = read_dataset(dataset_path)
            ufs_solve_dataset(dp, config, queries_num, seed, save_path)
