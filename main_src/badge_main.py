...
import csv
from methods.active_learning.badge import BadgeSampling
...
def badge_solve_dataset(dp, cfg, queries_num, seed, save_path):
    np.random.seed(seed)
    random.seed(seed)
    ...
    while True:
        iteration += 1
        model = BadgeSampling(dataset.emb, dataset.pseudo_label, np.array(queried_mask), None, None, None) 
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        for seed in seeds:            
            ...
            save_path = os.path.join(os.path.join(os.path.join(result_folder_path, "badge"), dataset), str(seed) + ".csv")
            if os.path.exists(save_path):
                continue
            dp, config = read_dataset(dataset_path)
            print(dataset, config["snr"])  
            badge_solve_dataset(dp, config, queries_num, seed, save_path)
